...
function generateEncryptionTable(array $substitutionSymbols, array $alphanumeric): array
{
    ...
    $alphanumericPointer = 0;

    foreach($substitutionSymbols as $symbolColumn) {
        foreach($substitutionSymbols as $symbolRow) {
            $encryptionTable[$symbolColumn][$symbolRow] = $alphanumeric[$alphanumericPointer];
            $alphanumericPointer++;
        }
    }
